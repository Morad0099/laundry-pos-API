...
export class AuthController {
  ...
  async login(credentials: LoginDTO): Promise<ApiResponse<any>> {
    try {
      ...
      return ResponseHandler.error("Login failed", 500, err.message);
    }
  }
  ...
  async refreshToken(token: string): Promise<ApiResponse<any>> {
    try {
      ...
      const err = error as ApiError;
      return ResponseHandler.error("Token refresh failed", 500, err.message);
    }
  }
  ...
  async forgotPassword(phone: string): Promise<ApiResponse<any>> {
    try {
      ...
      const err = error as ApiError;
      return ResponseHandler.error("Password reset failed", 500, err.message);
    }
  }
  ...
  async logout(
    ...
    try {
      ...
      const err = error as ApiError;
      return ResponseHandler.error("Logout failed", 500, err.message);
    }
  }
}
