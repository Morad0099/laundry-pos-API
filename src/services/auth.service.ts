...
import { LoginDTO, AuthResponse, TokenPayload, AdminInterface } from '../types/admin.types';
...
export class AuthService {
    ...
    async login(credentials: LoginDTO): Promise<AuthResponse | null> {
        const admin = await AdminModel.findOne({ phone: credentials.phone })
            .select('+password');
    ...
    async refreshToken(token: string): Promise<{ accessToken: string } | null> {
        try {
            ...
            const admin = await AdminModel.findById(payload.id).select('+refreshToken');
    ...
    async logout(adminId: string, accessToken: string, refreshToken: string): Promise<boolean> {
        try {
